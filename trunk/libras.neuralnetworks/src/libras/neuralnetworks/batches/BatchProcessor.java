...
public class BatchProcessor
{
	 ...
	 */
	public BatchProcessResult[] process(Batch batch)
	{
		BatchProcessResult[] results = new BatchProcessResult[batch.size()];
		...
		for (int i = 0; i < batch.size(); i++)
		{
			...
			try
			{
				...
				results[i] = new BatchProcessResult(true, item.getBatchName());
			}
			catch (Exception e)
			{
				results[i] = new BatchProcessResult(e, item.getBatchName());
			}
		}
	 ...
	 */
	private void executeItem(BatchItem batchItem) throws Exception
	{
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private void getTrainingData(BatchItem batchItem,
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private DistanceNeuralNetwork getNetworkSetup(BatchItem batchItem) throws Exception
	{
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private void teachNetwork(DistanceNeuralNetwork network, List<Pair<String, double[]>> trainingData, BatchItem batchItem) throws Exception
	{
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private void validateNetwork(DistanceNeuralNetwork network, List<Pair<String, double[]>> validationData, BatchItem batchItem) throws Exception
	{
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private void createReportDataFile(BatchItem batchItem, double[][] input, String[] expectedOutput, String[] computedOutput) throws Exception
	{
		double hitTax = 0.0;
	 ...
	 * @throws Exception When an unexpected exception occurs.
	 */
	private void createNetworkFile(BatchItem batchItem, DistanceNeuralNetwork network) throws Exception
	{
		File directory = batchItem.getReportDataFile().getParentFile();
		if (!directory.exists()) directory.mkdirs();
